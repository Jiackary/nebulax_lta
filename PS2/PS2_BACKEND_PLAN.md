...
`build_status` now re-plans when a live outage blocks a door the stored plan uses, persists the
result, and reports `rerouted: true`. Verified end to end: Exit 6 → Exit 7, leave 09:19 → 09:14,
and `steps_plain` follows. The `/offline` bundle picks this up because it builds its snapshot
