...
| T18 | **DataMall's auth failures split two ways — re-measured 17 Sep 2026.** A **wrong** key returns a clean `401` with an empty body. A **missing or empty** `AccountKey` header returns `404 "The requested API was not found"`, byte-identical to what a genuinely wrong endpoint returns. So `401` means the key is bad, while `404` stays ambiguous: check the header is actually being sent before assuming the URL is wrong. The earlier note that a wrong key looks like a wrong URL no longer holds. | Measured across five header conditions, 17 Sep 2026; handled in `backend/app/sources/datamall.py` |
...
Consolidated product, backend, speech, and routing roadmap: [PS2_CONSOLIDATED_ROADMAP.md](PS2_CONSOLIDATED_ROADMAP.md).
Independent UX review, wireframes, and smaller-model polish handoff: [PS2_UX_POLISH_PLAN.md](PS2_UX_POLISH_PLAN.md).
