...
def test_scheduled_check_only_sends_to_subscriptions_for_that_trip(monkeypatch):
    ...
    monkeypatch.setattr(
        jobs,
        "send_push",
        lambda subscription, _: (sent_to.append(subscription["endpoint"]) or True, None),
    )
    monkeypatch.setattr(
        jobs.store,
        "mark_sent",
...
def test_failed_delivery_is_not_marked_sent(monkeypatch):
    ...
    monkeypatch.setattr(jobs.notify, "check_trip", check_trip)
    monkeypatch.setattr(jobs.store, "was_sent", lambda *_: False, raising=False)
    monkeypatch.setattr(jobs, "send_push", lambda *_: (False, "temporary failure"))
    monkeypatch.setattr(
        jobs.store,
        "mark_sent",
        lambda *args: (marked.append(args) or True),
...
def test_partial_delivery_failure_is_not_marked_sent(monkeypatch):
    ...
    monkeypatch.setattr(
        jobs,
        "send_push",
        lambda subscription, _: (subscription["endpoint"] == "works", "temporary failure"),
    )
    monkeypatch.setattr(
        jobs.store,
        "mark_sent",
